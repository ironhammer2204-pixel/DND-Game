...
function getTrustedFrontendOrigin(req: { query: Record<string, unknown>; headers: Record<string, unknown> }): string {
  ...
  const candidates = [configuredOrigin, requested, headerOrigin].filter(Boolean) as string[];

  for (const candidate of candidates) {
    try {
      const url = new URL(candidate);
      if (url.protocol === "http:" || url.protocol === "https:") {
        // If we have a configured origin, ONLY allow exact matches or subdomains
        if (configuredOrigin) {
          const allowed = new URL(configuredOrigin);
          // Allow exact match or *.vercel.app subdomains
          if (url.origin === allowed.origin || url.hostname.endsWith('.vercel.app')) {
            return url.origin;
          }
        } else {
          return url.origin;
...
router.get("/google", async (req, res) => {
  try {
    ...
    console.log("[Auth] Origin detected:", origin);
