...
router.get("/:campaignId/factions", authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const factionsRes = await pool.query(
      "SELECT * FROM public.factions WHERE campaign_id = $1 ORDER BY power_level DESC, name",
      [campaignId]
    );

    const nemesesRes = await pool.query(
      "SELECT id, name, epithet, tier, status, grudge_score, faction_id FROM public.nemeses WHERE campaign_id = $1 AND faction_id IS NOT NULL",
      [campaignId]
    );

    const nemesesByFaction = new Map<string, typeof nemesesRes.rows>();
    for (const n of nemesesRes.rows) {
      const list = nemesesByFaction.get(n.faction_id) || [];
      list.push(n);
      nemesesByFaction.set(n.faction_id, list);
    }

    const factions = factionsRes.rows.map((f) => ({
      ...f,
      nemeses: nemesesByFaction.get(f.id) || [],
    }));
...
router.post("/:campaignId/factions", authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    if (!(await isCampaignDM(userId, campaignId))) {
      res.status(403).json({ error: "Only the DM can create factions." });
      return;
    }

    if (!name?.trim()) {
      res.status(400).json({ error: "Faction name is required." });
      return;
    }

    const result = await pool.query(
      "INSERT INTO public.factions (campaign_id, name, disposition, power_level, description) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [campaignId, name.trim(), disposition, Math.max(1, power_level), description || null]
    );
